...
const getJsonFormatRequirement = (schema: unknown): string => {
  // 分析Schema的type字段来判断输出类型
  let outputType = '数据';
  let startChar = '{';
...
export default {
  getStep1AnalyzerPrompt: (rawInput: string): string => `
...
${getJsonFormatRequirement(structuredDataSchema)}
...
${rawInput}`,
  ...
  getStep2DesignerPrompt: (context: { industry: string; preference: string }): string => `
...
${getJsonFormatRequirement(designSystemSchema)}

输出必须严格符合以下 JSON Schema:
${getSchemaString(designSystemSchema)}`,

  getStep3ArchitectPrompt: (context: { companyName: string; products: { name: string }[] }): string => `
...
${getJsonFormatRequirement(websiteArchitectureSchema)}
...
${getJsonFormatRequirement(contentCompleteBlueprintSchema)}
...
${getJsonFormatRequirement(blockSchema)}
...
${getJsonFormatRequirement(taskList)}
